...
def make_arrow(root, dataset_root):
    train_data = list(
        map(json.loads, open(f"{root}/train.json").readlines())
    )
    test1_data = list(
        map(json.loads, open(f"{root}/test1.json").readlines())
    )
    dev_data = list(map(json.loads, open(f"{root}/dev.json").readlines()))
