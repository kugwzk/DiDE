    ...
    for split in ["train", "val"]:
        for k, v in pl_module.hparams.config["loss_names"].items():
            ...
            if k == "vqa":
                ...
                if pl_module.hparams.config["use_dst"] and split == "train":
                    setattr(pl_module, f"{split}_soft_{k}_loss", Scalar())
                    setattr(pl_module, f"{split}_attention_{k}_loss", Scalar())
            elif k == "nlvr2":
                if split == "train":
                    setattr(pl_module, f"train_{k}_accuracy", Accuracy())
                    setattr(pl_module, f"train_{k}_loss", Scalar())
                    if pl_module.hparams.config["use_dst"]:
                        setattr(pl_module, f"{split}_soft_{k}_loss", Scalar())
                        setattr(pl_module, f"{split}_attention_{k}_loss", Scalar())
                else:
...
def epoch_wrapup(pl_module):
    ...
    for loss_name, v in pl_module.hparams.config["loss_names"].items():
        ...
        if loss_name == "vqa":
            ...
            if phase == 'train' and pl_module.hparams.config["use_dst"]:    # add dst part loss
                pl_module.log(
                    f"{loss_name}/{phase}/soft_loss_epoch",
                    getattr(pl_module, f"{phase}_soft_{loss_name}_loss").compute(),
                )
                getattr(pl_module, f"{phase}_soft_{loss_name}_loss").reset()
                pl_module.log(
                    f"{loss_name}/{phase}/attention_loss_epoch",
                    getattr(pl_module, f"{phase}_attention_{loss_name}_loss").compute(),
                )
                getattr(pl_module, f"{phase}_attention_{loss_name}_loss").reset()
            ...
            if phase == "train":
                ...
                if pl_module.hparams.config["use_dst"]:    # add dst part loss
                    pl_module.log(
                        f"{loss_name}/{phase}/soft_loss_epoch",
                        getattr(pl_module, f"{phase}_soft_{loss_name}_loss").compute(),
                    )
...
def set_schedule(pl_module):
    ...
    head_names = ["vqa_classifier", "nlvr2_classifier", "ve_classifier"]
