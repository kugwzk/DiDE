...
class Block(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        _x, attn, q, k, v = self.attn(norm1_x, mask=mask)
        x = x + self.drop_path(_x)
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        return x, attn, norm1_x, q, k, v
...
class VisionTransformer(nn.Module):
    ...
    def visual_embed(self, _x, max_image_len=200, mask_it=False, pre_select=None,):
        _, _, ph, pw = self.patch_embed.proj.weight.shape
        ...
        if mask_it:
            return x, x_mask, (patch_index, (H, W)), label, select
        else:
            return x, x_mask, (patch_index, (H, W)), None, select
...
@register_model
def vit_mini_patch32_384(pretrained=False, **kwargs):
    """ mini VIT model. Depth=6, heads=12, embed_dim=384, mlp_ratio=4"""
    model_kwargs = dict(patch_size=32, embed_dim=384, depth=6, num_heads=12, **kwargs)
    model = _create_vision_transformer(
        "vit_mini_patch32_384", pretrained=pretrained, **model_kwargs
    )
    return model

@register_model
def vit_tiny_patch32_384(pretrained=False, **kwargs):
    """ tiny VIT model. Depth=6, embed_dim=768, heads=12, mlp_ratio=4"""
    model_kwargs = dict(patch_size=32, embed_dim=768, depth=6, num_heads=12, **kwargs)
    model = _create_vision_transformer(
        "vit_tiny_patch32_384", pretrained=pretrained, **model_kwargs
    )
    return model
